    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-2">
        ...
        <div className="lg:col-span-3 space-y-2">
          {/* Style Panel */}
          <div className="glass p-3 rounded-lg">
            <h3 className="font-bold text-sm mb-2">🎨 Style</h3>

            <div className="mb-2">
              <label className="text-xs text-gray-400">Color</label>
              <div className="flex flex-wrap gap-1 mt-1">
                {PRESET_COLORS.map((color) => (
                  <button key={color} onClick={() => setDrawColor(color)}
                    className={`w-7 h-7 rounded border-2 ${drawColor === color ? 'border-cyan-400 scale-110' : 'border-transparent'}`}
                    style={{ backgroundColor: color }} />
                ))}
              </div>
              <input type="color" value={drawColor} onChange={(e) => setDrawColor(e.target.value)} className="w-full h-8 mt-1 rounded cursor-pointer" />
            </div>

            <div className="mb-2">
              <label className="text-xs text-gray-400">Brush: {drawSize}px</label>
              <input type="range" min="1" max="50" value={drawSize} onChange={(e) => setDrawSize(Number(e.target.value))} className="w-full" />
            </div>

            <div className="mb-2">
              <label className="text-xs text-gray-400">Font</label>
              <select value={selectedFont} onChange={(e) => setSelectedFont(e.target.value)} className="text-xs">
                {FONTS.map((f) => <option key={f} value={f} className="bg-gray-900">{f}</option>)}
              </select>
            </div>
            ...
            <div>
              ...
              <input type="range" min="8" max="200" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className="w-full" />
            </div>

            <div className="mt-2">
              <label className="text-xs text-gray-400">Zoom: {Math.round(zoom * 100)}%</label>
              <input type="range" min="20" max="150" value={zoom * 100} onChange={(e) => setZoom(Number(e.target.value) / 100)} className="w-full" />
            </div>
          </div>
          ...
          {showAi && (
            <div className="glass p-3 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setShowAi(false)} className="text-xs">✕</button>
              </div>
              <div className="text-xs whitespace-pre-wrap max-h-40 overflow-y-auto">{aiSuggestion}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="text-xs text-center text-gray-400">
        ✅ Auto-saves every 3 seconds • Touch-screen compatible • Use 🔴 Laser for presentations
      </div>
    </div>
  );
    }
